<div class="container mt-20 pb-20 md:pb-8">
    ...
    <div class="px-2 flex flex-col gap-4">
        ...
        <div class="bg-background-secondary rounded-lg p-4">
            ...
            <div class="flex flex-row items-center justify-between py-2 border-b border-neutral/20">
                <div>
                    <p class="text-sm text-base/70">{{ $session->ip_address }} -
                        {{ $session->last_activity->diffForHumans() }}</p>
                    <p class="text-sm text-base/40">{{ $session->formatted_device }}</p>
                </div>
                ...
                @else
                <span class="text-sm text-base/50 italic">{{ __('account.current_device') }}</span>
                @endif
            </div>
            @endforeach
        </div>
        ...
        <div class="bg-background-secondary rounded-lg p-4">
            ...
            @if ($twoFactorEnabled)
            <p class="text-sm text-base/70">{{ __('account.two_factor_authentication_enabled') }}</p>
            ...
            @else
            <p class="text-sm text-base/70">{{ __('account.two_factor_authentication_description') }}</p>
            ...
            <x-modal :title="__('account.two_factor_authentication_enable')" open="true">
                ...
                <div class="flex flex-col items-center mt-4">
                    ...
                    <p class="text-base/40 mt-2 text-sm text-center">
                ...
                <x-slot name="closeTrigger">
                    <button @click="document.location.reload()" class="text-base/70">
