{{-- MoltHost — Editorial Footer --}}
@php
    $appName = config('app.name', 'MoltHost');
    $splitName = preg_match('/^([A-Za-z]+?)([A-Z][a-z]+)$/', $appName, $nameParts);
@endphp
<footer class="w-full px-4 py-12 mt-auto bg-background-secondary/40 border-t border-neutral/15">
    <div class="container">
        <div class="grid grid-cols-1 lg:grid-cols-[1.4fr_1fr_1fr_1fr] gap-10 mb-10">
            {{-- Brand column --}}
            <div class="flex flex-col gap-4 max-w-sm">
                <div class="flex flex-row gap-2.5 items-center">
                    <x-logo class="h-7" />
                    @if(theme('logo_display', 'logo-and-name') != 'logo-only')
                    <span class="text-lg font-semibold leading-none tracking-tight select-none">
                        @if($splitName)
                            {{ $nameParts[1] }}<span class="text-primary">{{ $nameParts[2] }}</span>
                        @else
                            {{ $appName }}
                        @endif
                    </span>
                ...
                <p class="text-sm text-muted leading-relaxed">
                    {{ theme('hero_subtitle', 'Database and LLM hosting that scales by molting, not by panicking. Built in Brazil, run worldwide.') }}
                </p>
                ...
                <div class="flex items-center gap-2 mt-1">
                    ...
                    <a href="{{ theme('discord_url') }}" target="_blank" rel="noopener noreferrer"
                       class="w-9 h-9 bg-background border border-neutral/20 rounded-lg flex items-center justify-center text-muted hover:text-primary hover:border-primary/40 transition-all">
            ...
            {{-- Product --}}
            <div>
                <div class="mh-mono text-[11px] text-muted uppercase tracking-[0.10em] mb-4">{{ __('molthost::messages.footer.product') }}</div>
                <ul class="flex flex-col gap-2 text-sm text-muted">
                    <li><a href="{{ route('home') }}#products" wire:navigate class="hover:text-base transition-colors">{{ __('molthost::messages.footer.link_databases') }}</a></li>
                    <li><a href="{{ route('home') }}#products" wire:navigate class="hover:text-base transition-colors">{{ __('molthost::messages.footer.link_llm') }}</a></li>
                    <li><a href="{{ route('home') }}#pricing" wire:navigate class="hover:text-base transition-colors">{{ __('molthost::messages.footer.link_pricing') }}</a></li>
                    <li><a href="{{ route('home') }}#regions" wire:navigate class="hover:text-base transition-colors">{{ __('molthost::messages.footer.link_regions') }}</a></li>
                </ul>
            </div>

            {{-- Developers --}}
            <div>
                <div class="mh-mono text-[11px] text-muted uppercase tracking-[0.10em] mb-4">{{ __('molthost::messages.footer.developers') }}</div>
                <ul class="flex flex-col gap-2 text-sm text-muted">
                    <li><a href="https://paymenter.org/docs" target="_blank" rel="noopener noreferrer" class="hover:text-base transition-colors">{{ __('molthost::messages.footer.link_docs') }}</a></li>
                    <li><a href="https://paymenter.org/docs/api" target="_blank" rel="noopener noreferrer" class="hover:text-base transition-colors">{{ __('molthost::messages.footer.link_api') }}</a></li>
                    <li><a href="https://paymenter.org/docs/cli" target="_blank" rel="noopener noreferrer" class="hover:text-base transition-colors">{{ __('molthost::messages.footer.link_cli') }}</a></li>
                    <li><a href="https://paymenter.org/docs/migrations" target="_blank" rel="noopener noreferrer" class="hover:text-base transition-colors">{{ __('molthost::messages.footer.link_migrations') }}</a></li>
                </ul>
            </div>

            {{-- Legal + Status --}}
            <div>
                <div class="mh-mono text-[11px] text-muted uppercase tracking-[0.10em] mb-4">{{ __('molthost::messages.footer.legal') }}</div>
                <ul class="flex flex-col gap-2 text-sm text-muted mb-5">
                    @if(config('settings.tos'))
                        <li><a href="{{ config('settings.tos') }}" target="_blank" rel="noopener noreferrer" class="hover:text-base transition-colors">{{ __('molthost::messages.footer.link_terms') }}</a></li>
                    @endif
                    @if(config('settings.privacy_policy'))
                        <li><a href="{{ config('settings.privacy_policy') }}" target="_blank" rel="noopener noreferrer" class="hover:text-base transition-colors">{{ __('molthost::messages.footer.link_privacy') }}</a></li>
                    @endif
                    <li><a href="mailto:dpa@{{ parse_url(config('app.url'), PHP_URL_HOST) }}" class="hover:text-base transition-colors">{{ __('molthost::messages.footer.link_dpa') }}</a></li>
                </ul>
                <div class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-success/10 border border-success/20 text-success text-xs font-medium">
                    <span class="w-1.5 h-1.5 rounded-full bg-success animate-pulse-slow"></span>
                    {{ __('molthost::messages.footer.systems_operational') }}
                </div>
            </div>
        </div>

        {{-- Meta row --}}
        <div class="pt-6 border-t border-neutral/15 flex flex-col md:flex-row items-start md:items-center justify-between gap-3 text-xs">
            <div class="mh-mono text-muted">
                © {{ date('Y') }} {{ config('app.name') }} · {{ __('molthost::messages.footer.billing_powered_by') }}
            </div>
            <div class="flex items-center gap-3">
                <span class="mh-mono text-muted">PT-BR · EN · ES</span>
                <a class="flex items-center gap-1.5 text-muted/60 hover:text-muted transition-colors" href="https://paymenter.org" target="_blank" rel="noopener noreferrer">
                    <span>{{ __('molthost::messages.footer.powered_by') }}</span>
                    <svg class="h-2.5 w-auto text-current" width="635" height="124" viewBox="0 0 635 124" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
