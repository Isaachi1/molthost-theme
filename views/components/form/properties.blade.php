    ...
    @switch($property->type)
            ...
            @foreach ($property->allowed_values as $value)
                <div class="flex items-center">
                    <input type="radio" value="{{ $value }}" name="properties.{{ $property->key }}" type="radio"
                        ...
                        class="form-radio size-4 text-primary rounded-full focus:ring-secondary hover:bg-secondary ring-offset-background focus:ring-2 bg-background-secondary border-neutral" />
                    <label class="ml-2 text-sm text-base/70"
        ...
        @case('select')
            <x-form.select name="properties.{{ $property->key }}" :label="$property->name"
                ...
                <option disabled value="" style="color: hsl(var(--c-muted))">{{ __('common.select.placeholder', ['name' => $property->name]) }}</option>
                @foreach ($property->allowed_values as $value)
                    <option style="color: hsl(var(--c-base))" value="{{ $value }}">{{ $value }}</option>
                @endforeach
            </x-form.select>
        @break
