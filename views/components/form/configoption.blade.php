<div class="flex flex-col gap-1">
    @switch($config->type)
        ...
        @case('slider')
            <div x-data="{
                ...
                <label for="{{ $name }}"
                    class="mb-1 text-sm text-base/70">
    ...
    @isset($config->description)
        @isset($config->link)
            <a href="{{ $config->link }}" class="text-xs text-base/50 hover:underline hover:text-secondary group">
                {{ $config->description }}
                <x-ri-arrow-right-long-line class="ml-1 size-3 inline-block -rotate-45 group-hover:rotate-0 transition" />
            </a>
        @else
            <p class="text-xs text-base/50">{{ $config->description }}</p>
        @endisset
    @endisset
</div>
