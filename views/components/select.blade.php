...
>
    <button
        ...
        :class="{ 'ring-1 ring-primary/50': selectOpen }"
        class="relative w-full min-h-[38px] py-2 pl-3 pr-10 text-left bg-background-secondary border border-neutral/20 rounded-lg shadow-sm cursor-pointer text-sm focus:outline-none focus:ring-1 focus:ring-primary/50 transition-all"
        type="button"
    >
        ...
        <span class="absolute inset-y-0 right-0 flex items-center pr-2 pointer-events-none">
            <x-ri-expand-up-down-line class="size-4 text-muted" />
        </span>
    </button>
    ...
    <ul
        ...
        class="absolute z-50 w-full py-1 overflow-auto bg-background-secondary border border-neutral/15 rounded-lg shadow-lg max-h-60 focus:outline-none text-sm"
        x-cloak
    >
        <template x-for="item in selectableItems" :key="item.value">
            <li
                ...
                :class="{ 'bg-primary/10': selectableItemIsActive(item) }"
                class="relative py-2 pl-8 pr-4 cursor-pointer select-none hover:bg-primary/5 transition-colors"
            >
                <span
                    class="block truncate"
                    :class="{ 'font-semibold text-primary': item.value === selectedValue }"
                    x-text="item.label"
                ></span>
                <span
